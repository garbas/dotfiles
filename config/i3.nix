...
let
  ...
  i3Theme = brightness: builtins.readFile "${theme}/i3.${brightness}";
  ...
  self= brightness: writeText "i3-config-${brightness}"
    ''
      ...
      bindsym $mod+Shift+s exec --no-startup-id "${theme_switch}/bin/theme-switch toggle"
      ...
      bar {
          status_command ${py3status}/bin/py3status -c /tmp/config/i3status
          position bottom
          tray_output $mon_lap
      #{{{ i3bar colors
          colors {
